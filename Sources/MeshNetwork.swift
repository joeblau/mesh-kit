...
public class MeshNetwork<T: Codable> {
    ...
    init(serviceType: String, displayName: String = MeshDevice.name, delegate: MeshNetworkDelegate, autoReconnect: Bool = true) {
        ...
        advertiserDelegate = MeshAdvertiserDelegate(session: session, meshNetwork: delegate)
        browserDelegate = MeshBrowserDelegate(session: session, meshNetwork: delegate, autoReconnect: autoReconnect)
    ...
    public func broadcast<T: Codable>(message: T, mode: MCSessionSendDataMode = .reliable) {
        if let data = try? jsonEncoder.encode(message).base64EncodedData() {
